...
collected = duplicate(collected, {'stop_time_nsec' 'thread_stop_nsec' 'thread_stop_nsec' 'none'}, {'global_time' 'thread_time' 'another_thread_stop_nsec', 'ones'}, 1); % Duplicate two columns. The new columns will be used to store the computed time difference between start and stop for both global and per-thread process.
collected = apply(collected, 'global_time', @time_difference_global); % Compute the global start-stop difference
collected = apply(collected, 'thread_time', @time_difference_thread); % Compute the start-stop time difference per threads
...
global_timing_100 = where(global_timing, {'entropy' 'ct'}, {[0.1] [100000000]}); % Select rows denoting experiments with an entropy of 0.1 and performing 100 million jumps.
global_timing_200 = where(global_timing, {'entropy' 'ct'}, {[0.1] [200000000]}); % Select rows denoting experiments with an entropy of 0.1 and performing 200 million jumps.
...
thread_timing_100 = where(thread_timing, {'entropy' 'ct'}, {[0.1] [100000000]}); % Select rows denoting experiments with an entropy of 0.1 and performing 100 million jumps.
thread_timing_200 = where(thread_timing, {'entropy' 'ct'}, {[0.1] [200000000]}); % Select rows denoting experiments with an entropy of 0.1 and performing 200 million jumps.
